...
/**
 * This component it used to display a single Poll
 */
const Poll = ({poll, handleVoteChange, currentVote, handleVoteSubmit}) => {
    const calculatePercentage = (choice) => {
        return poll.totalVotes === 0
            ? 0 : (choice.voteCount * 100) / (poll.totalVotes)
    }
    ...
    const pollChoices = []
    /* Show the calculated results only when the user has voted or the poll is expired */
            ...
            <div className="poll-footer">
                {
                    !(poll.selectedChoice || poll.expired) ?
                        (<Button className="vote-button" disabled={!currentVote}
                                 onClick={handleVoteSubmit}>Vote</Button>) : null
                }
...
const CompletedOrVotedPollChoice = ({percentVote, choice, isSelected, isWinner}) => {
    return (
        <div className="cv-poll-choice">
            <span className="cv-poll-choice-details">
                ...
                {isSelected ? (
                    <CheckCircleTwoTone  style={{marginLeft: "10px", marginTop: "5px"}}/>
                ) : null}
            </span>
